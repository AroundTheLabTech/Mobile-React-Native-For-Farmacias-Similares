...
import BarChart9InchesStyles from './style/BarChartStyles';
...
interface GroupedBarChart9InchesProps {
  data: CategoryData[];
  barColor?: string;
  listOfColors: string[];
}

const BarChart9Inches: React.FC<GroupedBarChart9InchesProps> = ({ data, listOfColors }) => {
  ...
  return (
    <View style={BarChart9InchesStyles.container}>
      <Svg height="250" width="100%">
        {data.map((e, index) => {
          ...
          return (
            <G key={index} x={x}>
              <Ellipse x={index + 10} y={12} cx={5} cy={5} rx={5} ry={5} fill={listOfColors[index]} />
              <SVGText
                x={25}
                y={25}
        ...
        {divideNumberIntoPortions(100).map((percentage, index) => {
          const y = chartHeight - (percentage / 100) * chartHeight + 55;
          ...
          return (
            <G key={categoryIndex} x={groupOffset}>
              ...
              {categoryData.stats.map((stat, statIndex) => {
                ...
                const y = chartHeight - barHeight + 55;
                ...
                return (
                  <G key={statIndex} x={x}>
                    ...
                    <SVGText
                      x={barWidth * 0.5}
                      y={y + barHeight + 25} // Ajusta la posición vertical de la etiqueta
                    ...
                    <SVGText
                      x={barWidth * 0.5}
                      y={y + barHeight + 45} // Ajusta la posición vertical de la etiqueta
                      fontSize={fontSizes.xxxs}
                      fill={colors.primary}
                      fontWeight={fontWeight.xxxl}
                      textAnchor="middle"
...
export default BarChart9Inches;
