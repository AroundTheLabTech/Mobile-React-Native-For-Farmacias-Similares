...
import {
  getListAvalibleCompetition,
  postSessionGame,
  putCompetitionSession,
...
const GameIframe: React.FC<Props> = ({navigation, route}) => {
  ...
  const {gameUrl, id, score} = route.params;
  ...
  const [_session, setSession] = useState<TGameSession | undefined>();
  ...
  useEffect(() => {
    if (__DEV__) {
      console.log(
        `[DEBUG GameIframe] Locking orientation -> id=${id}, isVertical=${isVerticalGame}`,
      );
    }
    if (isVerticalGame) {
  ...
  const injectedJS = `
    (function(){
      // Bloquear scroll y gestos del navegador
      ...
      if (!window.__SimiBridge) {
        // Cola para evitar pérdida de mensajes rápidos en el puente WebView -> Nativo
        var msgQueue = [];
        var sending = false;
        function processQueue() {
          if (sending || msgQueue.length === 0) return;
          sending = true;
        ...
        window.__SimiBridge = (function(){
          ...
          return {
            ...
            reset: function(){ scoreHistory = []; previousScore = 0; msgQueue = []; sending = false; },
          };
        })();
        ...
        window.addEventListener('message', function(ev){
          try {
            ...
            if (Object.keys(payload).length) safePost(JSON.stringify(payload));
          } catch(e){}
        }, false);
        ...
        window.postMessage = function(msg, target){
          ...
          if (Object.keys(p).length) try { safePost(JSON.stringify(p)); } catch(e){}
  ...
  const handleMessage = (event: WebViewMessageEvent) => {
    try {
      ...
      if (__DEV__) {
        console.log(
          `[DEBUG GameIframe] onMessage -> type=${typeValue}, score=${scoreValue}, number=${numberValue}, game=${id}`,
        );
      }
      ...
      // Check type filter
      if (strategy.requireType && typeValue !== strategy.requireType) {
        return;
      }
      ...
      if (strategy.mode === 'direct') {
        ...
        const normalized =
          scoreValue > 100 ? (scoreValue - 10) / (strategy.divisor || 1) : 1;
        ...
        if (scoreValue <= 0 && !strategy.dedup) {
          return;
        }
        ...
        if (strategy.divisor) {
          ...
          if (strategy.normalizeMin != null) {
            v = v > strategy.normalizeMin ? v : strategy.normalizeMin;
          }
        }
      ...
      if (delta > 0 && !appliedAbsolute) {
        ...
        setCurrentScore(next);
        console.log(
          `[DEBUG GameIframe] Score updated -> delta=${delta}, next=${next}`,
        );
      }
    } catch (e) {
  ...
  const saveScoreDelta = useCallback(
    async (navigateBack = false) => {
      ...
      try {
        ...
        if (__DEV__) {
          console.log(
            `[DEBUG GameIframe] saveScoreDelta -> latestScore=${latestScore}, lastSaved=${lastSavedScoreRef.current}, delta=${deltaToSend}, navigateBack=${navigateBack}`,
          );
        }
        ...
        if (deltaToSend > 0) {
          ...
          const postPromise = (async () => {
            ...
            try {
              ...
              console.log(
                `[DEBUG GameIframe] Posting session -> game=${gameNumber}, score=${deltaToSend}`,
              );
              const response = await postSessionGame(newGameSession);
              console.log(
                `[DEBUG GameIframe] Session response -> session_id=${response?.session_id}`,
              );
              ...
              if (response?.session_id) {
                try {
                  ...
                  console.log(
                    '[DEBUG GameIframe] Error adding competition session:',
                    compErr,
                  );
                }
              ...
              if (__DEV__) {
                console.log('[DEBUG GameIframe] Error in postPromise:', err);
              }
              throw err;
            } finally {
          ...
          console.log(
            `[DEBUG GameIframe] No delta to save (deltaToSend=${deltaToSend})`,
          );
        }
      } finally {
        setIsSaving(false);
      ...
      if (navigateBack) {
        console.log(
          `[DEBUG GameIframe] Navigating back, refreshing data, optimisticScore=${currentScoreRef.current}`,
        );
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener?.('beforeRemove', (e: any) => {
      if (isNavigatingBackRef.current) {
        console.log(
          '[DEBUG GameIframe] beforeRemove -> already navigating back via saveScoreDelta, allowing navigation',
        );
        return;
      }
      ...
      if (isPostingRef.current) {
        console.log(
          '[DEBUG GameIframe] beforeRemove -> already posting, allowing navigation',
        );
        return;
      }

      const delta = Math.round(
        Math.max(0, currentScoreRef.current - lastSavedScoreRef.current),
      );
      if (__DEV__) {
        console.log(
          `[DEBUG GameIframe] beforeRemove -> delta=${delta}, currentScore=${currentScoreRef.current}, lastSaved=${lastSavedScoreRef.current}`,
        );
      }
      if (delta > 0) {
        ...
        e.preventDefault();
        console.log(
          '[DEBUG GameIframe] beforeRemove -> preventing navigation, saving first',
        );
        saveScoreDelta(true);
      } else {
        // No hay delta pendiente, refrescar datos y dejar pasar
        console.log(
          '[DEBUG GameIframe] beforeRemove -> no delta, passing through',
        );
  ...
  if (__DEV__) {
    console.log(`[DEBUG GameIframe] gameUrl=${gameUrl}`);
  }
